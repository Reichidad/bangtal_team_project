...
money_control = MoneyControl(10000)
money_control.set_money_gui(scene_table,100,100,"./images/number/")
chip_list = []
...
def click_hit(x, y, action):
    ...
    if score > 21:
        showMessage(" --- BURST --- \n" + "Lose " + str(money_control.bet_money) + "$")
        clear_scene()
...
def click_stand(x, y, action):
    ...
    while d_score < 17:
        dealers_cards.append(get_cards())
        d_score = calc_score(dealers_cards)
        showMessage("My money : " + str(money_control.get_money()) + "$")
    if d_score > 21:
        money_control.calc_money(2)
        showMessage("Dealer Burst!!\n Get " + str(money_control.bet_money * 2) + "$"+ " My money : " + str(money_control.get_money()) + "$\n" + str_score())
    elif d_score > p_score:
        showMessage("Dealer Win!!\n Lose " + str(money_control.bet_money) + "$"+ " My money : " + str(money_control.get_money()) + "$\n" +str_score())
    elif d_score < p_score:
        money_control.calc_money(2)
        showMessage("Player Win!!\n Get " + str(money_control.bet_money * 2) +"$"+ " My money : " + str(money_control.get_money()) + "$\n" +str_score())
    else:
        money_control.calc_money(1)
        showMessage("Draw\n" + " My money : " + str(money_control.money) + "$")

    hide_chip()
    hide_cards()
    show_cards(0)
    clear_scene()
    money_control.print_state()
...
def click_bet(x, y, action):
    ...
    money_control.add_bet_money(25)
    money_control.add_chip()
    money_control.show_chip(scene_table)
